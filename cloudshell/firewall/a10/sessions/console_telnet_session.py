...
class ConsoleTelnetSession(TelnetSession):
    ...
    def __init__(self, host, username, password, port=None, on_session_start=None,
                 start_with_new_line=None, *args, **kwargs):
        super(ConsoleTelnetSession, self).__init__(
            host, username, password, port, on_session_start, loop_detector_max_action_loops=5,
            *args, **kwargs)
    ...
    def connect(self, prompt, logger):
        ...
        if self._handler.get_socket() is None:
            raise TelnetSessionException(
                ...
                "Failed to open telnet connection.")
        ...
        try:
            out = self.hardware_expect(
                cmd, expected_string=prompt, timeout=self._timeout, logger=logger,
